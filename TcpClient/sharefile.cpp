    ...
    : QWidget{parent}
{
    ...
    setLayout(pMainVBL);
    //槽函数
    connect(m_pCancleSelectPB,SIGNAL(clicked(bool)),this,SLOT(cancelSelect()));//取消选择
    connect(m_pSelectAllPB,SIGNAL(clicked(bool)),this,SLOT(selectAll()));   //全选
    connect(m_pOKPB,SIGNAL(clicked(bool)),this,SLOT(okShare()));   //确定
    connect(m_pCanclePB,SIGNAL(clicked(bool)),this,SLOT(cancelShare()));   //取消
}
